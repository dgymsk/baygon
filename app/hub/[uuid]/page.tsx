...
export default async function HubEventoPage({ params }: { params: Promise<{ uuid: string }> }) {
  ...
  const rows = (await sql`
    SELECT e.id::int AS evento_id, p.message_id, COALESCE(e.preset_id, p.preset_id)::int AS preset_id,
           ...
           r.resultado, r.war_id::int AS war_id, e.servidores
    FROM evento e
  ...
  `) as { evento_id: number; message_id: string | null; preset_id: number | null; uuid: string; titulo: string; titulo_raw: string | null; tamanho_max: number | null; intencao_fechada: boolean; tipo: string; tier: Tier | null; exige_registro: boolean; data: string; status: string; resultado: string | null; war_id: number | null; servidores: string[] }[];
  ...
  const vm = (userId: string, familia: string | null, chaveRaw: string | null): JogadorVM => {
    ...
    return {
      ...
      // 6 quadrados (node wars) + 1 bola (siege), pra decidir escalação com o passado à vista
      semana: semana.porChave.get(chave) ?? { nodewars: [], siege: null },
    };
  };

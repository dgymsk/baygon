...
export default async function HubConfigPage() {
  const [funcoes, parties, presets, membros, players, canais, meta, roles, emojis, canEdit] = await Promise.all([
    listFuncoes(), listParties(), listPresets(), listPlayerFuncoes(), listPlayers(), getIntencaoConfig(), getGuildMeta(), listarRolesGuild(), listarEmojisGuild(), canEditNow(),
  ]);
  ...
  return <ConfigBoard funcoes={funcoes} parties={parties} presets={presets} membros={membros} jogadores={jogadores} canais={canais} guildas={meta.guildas} roles={roles} emojis={emojis} canEdit={canEdit} />;
}

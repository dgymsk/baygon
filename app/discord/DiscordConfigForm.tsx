...
export default function DiscordConfigForm({ initial, canEdit }: { initial: DiscordConfig; canEdit: boolean }) {
  ...
  async function salvar() {
    ...
    try {
      const body = { guildId, staffRoleIds: staff.split(",").map((s) => s.trim()).filter(Boolean), confirmNodewar: cnw, confirmSiege: csg };
      ...
      setGuildId(dc.guildId); setStaff(dc.staffRoleIds.join(", ")); setCnw(dc.confirmNodewar); setCsg(dc.confirmSiege);

...
export default function DiscordConfigForm({ initial, canEdit }: { initial: DiscordConfig; canEdit: boolean }) {
  ...
  const [pendRole, setPendRole] = useState(initial.pendenteRoleId);
  ...
  async function salvar() {
    ...
    try {
      const body = { guildId, staffRoleIds: staff.split(",").map((s) => s.trim()).filter(Boolean), confirmNodewar: cnw, confirmSiege: csg, logChannel: log, reportChannel: rep, registroRoleId: regRole, registerChannel: regChan, pendenteRoleId: pendRole };
      ...
      setGuildId(dc.guildId); setStaff(dc.staffRoleIds.join(", ")); setCnw(dc.confirmNodewar); setCsg(dc.confirmSiege); setLog(dc.logChannel); setRep(dc.reportChannel); setRegRole(dc.registroRoleId); setRegChan(dc.registerChannel); setPendRole(dc.pendenteRoleId);
      ...
      <div style={{ maxWidth: 720, margin: "0 auto" }}>
        ...
        <div style={{ ...card, display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, borderTop: `1px solid ${C.borderSoft}`, paddingTop: 12 }}>
            ...
            <div><label style={label} title="a staff dá este cargo pra quem precisa se registrar; a jornada TIRA quando o registro termina (depois de dar o cargo da guilda)">Cargo de registro pendente (ID)</label><input value={pendRole} readOnly={ro} onChange={(e) => setPendRole(e.target.value.replace(/[^0-9]/g, ""))} placeholder="ID do cargo (vazio = não usa)" style={input} /></div>
          ...
          <div style={dica}>Jornada de registro: cargo dado ao concluir + canal onde o /register funciona. ⚠ O bot precisa de <b>Gerenciar Cargos</b> e <b>Gerenciar Apelidos</b>, e os cargos de registrado, de registro pendente e os cargos de guilda (em /guildas) devem ficar <b>abaixo</b> do cargo do bot — dar E tirar cargo exigem isso.</div>
        </div>

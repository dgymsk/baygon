...
export async function POST(req: Request) {
  ...
  const tpl = await criarTemplate(b.nome, b.tipo, b.tamanhoMax, b.ptIds);
...
export async function PATCH(req: Request) {
  const unauth = await requireEditor(); if (unauth) return unauth;
  const b = await corpo(req); if (!b) return NextResponse.json({ error: "JSON inválido" }, { status: 400 });
  await atualizarTemplate(b.id, { nome: b.nome, tipo: b.tipo, tamanhoMax: b.tamanhoMax, ptIds: b.ptIds });

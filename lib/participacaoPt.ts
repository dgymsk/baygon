...
export type Template = { id: number; nome: string; tipo: string; tamanho_max: number | null; pts: number[] };
...
export async function listPts(): Promise<Pt[]> {
  return (await sql`SELECT id, nome, COALESCE(emoji,'') AS emoji FROM participacao_pt ORDER BY nome`) as Pt[];
}
export async function criarPt(nome: unknown, emoji: unknown): Promise<Pt | null> {
  ...
  const rows = (await sql`INSERT INTO participacao_pt (nome, emoji) VALUES (${n}, ${emj}) RETURNING id, nome, COALESCE(emoji,'') AS emoji`) as Pt[];
  return rows[0] ?? null;
}
...
export async function listMembros(tipo?: string): Promise<Membro[]> {
  return (tipo
    ? await sql`SELECT tipo, chave, familia, pt_id FROM participacao_membro WHERE tipo = ${tipo}`
    : await sql`SELECT tipo, chave, familia, pt_id FROM participacao_membro`) as Membro[];
}
...
// ---------- Templates ----------
async function idsValidos(ptIds: unknown): Promise<number[]> {
  const validos = new Set((await listPts()).map((p) => p.id));
  return [...new Set((Array.isArray(ptIds) ? ptIds : []).map((x) => Math.trunc(Number(x))).filter((x) => validos.has(x)))];
}
export async function listTemplates(): Promise<Template[]> {
  const tpls = (await sql`SELECT id, nome, tipo, tamanho_max FROM participacao_template ORDER BY tipo, nome`) as Omit<Template, "pts">[];
  const rels = (await sql`SELECT template_id, pt_id FROM participacao_template_pt ORDER BY ordem, pt_id`) as { template_id: number; pt_id: number }[];
  const porTpl = new Map<number, number[]>();
  for (const r of rels) { const a = porTpl.get(r.template_id) ?? []; a.push(r.pt_id); porTpl.set(r.template_id, a); }
  return tpls.map((t) => ({ ...t, pts: porTpl.get(t.id) ?? [] }));
}
export async function getTemplate(id: number): Promise<Template | null> {
  const rows = (await sql`SELECT id, nome, tipo, tamanho_max FROM participacao_template WHERE id = ${id}`) as Omit<Template, "pts">[];
  if (!rows[0]) return null;
  const rels = (await sql`SELECT pt_id FROM participacao_template_pt WHERE template_id = ${id} ORDER BY ordem, pt_id`) as { pt_id: number }[];
  return { ...rows[0], pts: rels.map((r) => r.pt_id) };
}
export async function criarTemplate(nome: unknown, tipo: unknown, tamanhoMax: unknown, ptIds: unknown): Promise<Template | null> {
  if (!ehTipo(tipo)) return null;
  const n = nomeOk(nome);
  if (!n) return null;
  const rows = (await sql`INSERT INTO participacao_template (nome, tipo, tamanho_max) VALUES (${n}, ${tipo}, ${limiteOk(tamanhoMax)}) RETURNING id, nome, tipo, tamanho_max`) as Omit<Template, "pts">[];
  const tpl = rows[0];
  if (!tpl) return null;
  const ids = await idsValidos(ptIds);
  if (ids.length) await sql.transaction(ids.map((pid, i) => sql`INSERT INTO participacao_template_pt (template_id, pt_id, ordem) VALUES (${tpl.id}, ${pid}, ${i}) ON CONFLICT DO NOTHING`));
  return { ...tpl, pts: ids };
}
export async function atualizarTemplate(id: unknown, patch: { nome?: unknown; tipo?: unknown; tamanhoMax?: unknown; ptIds?: unknown }): Promise<void> {
  const tid = Math.trunc(Number(id));
  if (!Number.isFinite(tid)) return;
  const n = nomeOk(patch.nome);
  if (!n || !ehTipo(patch.tipo)) return;
  const ids = await idsValidos(patch.ptIds);
  await sql.transaction([
    ...
    ...ids.map((pid, i) => sql`INSERT INTO participacao_template_pt (template_id, pt_id, ordem) VALUES (${tid}, ${pid}, ${i}) ON CONFLICT DO NOTHING`),
  ]);
}

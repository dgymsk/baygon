...
export type Preset = { id: number; nome: string; tipo: string; tamanho_max: number | null; parties: PresetParty[] };
...
export async function listPresets(): Promise<Preset[]> {
  const ps = (await sql`SELECT id::int AS id, nome, tipo, tamanho_max FROM intencao_preset ORDER BY nome`) as Omit<Preset, "parties">[];
  if (!ps.length) return [];
...
export async function getPreset(id: number): Promise<Preset | null> {
  const rows = (await sql`SELECT id::int AS id, nome, tipo, tamanho_max FROM intencao_preset WHERE id = ${id}`) as Omit<Preset, "parties">[];
...
export async function atualizarPreset(id: unknown, patch: { nome?: unknown; tipo?: unknown; parties?: unknown; tamanhoMax?: unknown }): Promise<void> {
  const pid = num(id);
  if (pid == null) return;

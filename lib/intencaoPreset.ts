...
export type Preset = { id: number; nome: string; tipo: string; tier: Tier | null; tamanho_max: number | null; canal_id: string | null; parties: PresetParty[] };
...
export async function listPresets(): Promise<Preset[]> {
  const ps = (await sql`SELECT id::int AS id, nome, tipo, tier, tamanho_max, canal_id FROM intencao_preset ORDER BY nome`) as Omit<Preset, "parties">[];
  if (!ps.length) return [];
...
export async function getPreset(id: number): Promise<Preset | null> {
  const rows = (await sql`SELECT id::int AS id, nome, tipo, tier, tamanho_max, canal_id FROM intencao_preset WHERE id = ${id}`) as Omit<Preset, "parties">[];
...
export async function criarPreset(nome: unknown, tipo: unknown, parties: unknown, tamanhoMax?: unknown, tier?: unknown): Promise<Preset | null> {
  ...
  const rows = (await sql`INSERT INTO intencao_preset (nome, tipo, tamanho_max, tier) VALUES (${n}, ${tipo}, ${maxOk(tamanhoMax)}, ${tierOk(tier)}) RETURNING id::int AS id`) as { id: number }[];
  const id = rows[0]?.id;
  if (!id) return null;
...
export async function atualizarPreset(id: unknown, patch: { nome?: unknown; tipo?: unknown; parties?: unknown; tamanhoMax?: unknown; canalId?: unknown; tier?: unknown }): Promise<void> {
  const pid = num(id);
  if (pid == null) return;
  ...
  // tier vazio é opção válida ("não informado"), então só mexe quando o campo veio no patch

...
export type Preset = { id: number; nome: string; tipo: string; funcoes: PresetFuncao[] };
export type MembroInt = { tipo: string; chave: string; familia: string; funcao_id: number };
...
function funcoesOk(raw: unknown): number[] {
  const arr = Array.isArray(raw) ? raw : [];
  const out: number[] = [];
  for (const x of arr) {
    const id = num(typeof x === "object" && x ? (x as { funcao_id?: unknown }).funcao_id : x);
...
export async function listPresets(): Promise<Preset[]> {
  ...
  if (!ps.length) return [];
  const vin = (await sql`SELECT preset_id::int AS preset_id, funcao_id::int AS funcao_id, ordem FROM intencao_preset_pt ORDER BY ordem, funcao_id`) as { preset_id: number; funcao_id: number; ordem: number }[];
  return ps.map((p) => ({ ...p, funcoes: vin.filter((v) => v.preset_id === p.id).map((v) => ({ funcao_id: v.funcao_id, ordem: v.ordem })) }));
}
...
export async function getPreset(id: number): Promise<Preset | null> {
  ...
  const funcoes = (await sql`SELECT funcao_id::int AS funcao_id, ordem FROM intencao_preset_pt WHERE preset_id = ${id} ORDER BY ordem, funcao_id`) as PresetFuncao[];
  return { ...rows[0], funcoes };
}

export async function criarPreset(nome: unknown, tipo: unknown, funcoes: unknown): Promise<Preset | null> {
  const n = nomeOk(nome);
  ...
  const id = rows[0]?.id;
  if (!id) return null;
  await gravarFuncoes(id, funcoesOk(funcoes));
  return getPreset(id);
}

export async function atualizarPreset(id: unknown, patch: { nome?: unknown; tipo?: unknown; funcoes?: unknown }): Promise<void> {
  const pid = num(id);
  if (pid == null) return;
  ...
  if (patch.funcoes !== undefined) await gravarFuncoes(pid, funcoesOk(patch.funcoes));
}
...
async function gravarFuncoes(presetId: number, funcaoIds: number[]): Promise<void> {
  ...
  for (let i = 0; i < funcaoIds.length; i++) {
    await sql`INSERT INTO intencao_preset_pt (preset_id, funcao_id, ordem) VALUES (${presetId}, ${funcaoIds[i]}, ${i})
...
export async function listMembrosInt(tipo?: string): Promise<MembroInt[]> {
  return (tipo
    ? await sql`SELECT tipo, chave, familia, funcao_id::int AS funcao_id FROM intencao_membro WHERE tipo = ${tipo} ORDER BY familia`
    : await sql`SELECT tipo, chave, familia, funcao_id::int AS funcao_id FROM intencao_membro ORDER BY familia`) as MembroInt[];
}

export async function adicionarMembroFuncao(tipo: unknown, familia: unknown, funcaoId: unknown): Promise<void> {
  if (!ehTipo(tipo)) return;
  const fam = famOk(familia);
  const chave = chaveNome(fam);
  const pid = num(funcaoId);
  if (!chave || pid == null) return;
  await sql`INSERT INTO intencao_membro (tipo, chave, familia, funcao_id) VALUES (${tipo}, ${chave}, ${fam}, ${pid})
    ON CONFLICT (tipo, chave, funcao_id) DO UPDATE SET familia = EXCLUDED.familia`;
}

export async function removerMembroFuncao(tipo: unknown, familia: unknown, funcaoId: unknown): Promise<void> {
  if (!ehTipo(tipo)) return;
  const chave = chaveNome(famOk(familia));
  if (!chave) return;
  const pid = num(funcaoId);
  // sem funcao_id → tira a pessoa de TODAS as PTs daquele tipo
  if (pid == null) await sql`DELETE FROM intencao_membro WHERE tipo = ${tipo} AND chave = ${chave}`;
  else await sql`DELETE FROM intencao_membro WHERE tipo = ${tipo} AND chave = ${chave} AND funcao_id = ${pid}`;
}

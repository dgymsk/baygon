...
export type TipoCfg = { channelId: string; titulo: string; mensagem: string; pingRoleId: string; agenda: Agenda };
...
function tipoPadrao(label: string): TipoCfg {
  return { channelId: "", titulo: `Participação — ${label}`, mensagem: "Vai participar da war hoje? Marque abaixo.", pingRoleId: "", agenda: { ativo: false, dias: [], hora: "20:00" } };
}

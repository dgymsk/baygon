...
export async function listPlayers(): Promise<PlayerRow[]> {
  return (await sql`
    SELECT p.nome_familia, p.grupo, p.classe_bdo, p.is_core, p.ativo,
           count(DISTINCT d.war_id)::int AS n_wars
    FROM players p
    LEFT JOIN desempenho d ON d.nome_familia = p.nome_familia
    GROUP BY p.nome_familia, p.grupo, p.classe_bdo, p.is_core, p.ativo
...
export async function addPlayer(nome: string, grupo: string, classe: string | null, ativo: boolean): Promise<boolean> {
  const rows = await sql`
    INSERT INTO players (nome_familia, grupo, is_core, classe_bdo, ativo)
    VALUES (${nome.trim()}, ${grupoOr(grupo)}, FALSE, ${classe?.trim() || null}, ${ativo})
...
export type PlayerUpdate = { nome_familia: string; grupo: string; classe_bdo: string | null; is_core: boolean; ativo: boolean };
...
export async function updatePlayers(updates: PlayerUpdate[]): Promise<void> {
  ...
  const queries = updates.map((u) => sql`
    ...
    SET grupo = ${grupoOr(u.grupo)}, classe_bdo = ${u.classe_bdo?.trim() || null},
        is_core = ${u.is_core}, ativo = ${u.ativo}

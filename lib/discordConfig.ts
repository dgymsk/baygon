 ...
 */
export type DiscordConfig = { guildId: string; staffRoleIds: string[]; confirmNodewar: string; confirmSiege: string };
...
function sanitizaStore(raw: unknown): DiscordConfig {
  ...
  return { guildId: dig(c.guildId), staffRoleIds: roles, confirmNodewar: dig(c.confirmNodewar), confirmSiege: dig(c.confirmSiege) };
}
